    ...
    <section id="attractions" className="py-20 md:py-32 bg-nyc-black relative overflow-hidden optimize-visibility" style={{ containIntrinsicSize: '1px 800px' }}>
      ...
      >
        {t.attractions.items.map((spot, idx) => (
            <div 
                key={spot.id} 
                className="flex-shrink-0 w-[280px] md:w-[450px] snap-center"
            >
                <Reveal delay={idx * 100} width="100%" variant="zoom">
                    <motion.div 
                        layoutId={`card-${spot.id}`}
                        className="group relative h-[450px] md:h-[600px] overflow-hidden bg-zinc-900 cursor-pointer transition-transform duration-200 hover:-translate-y-2 rounded-lg"
                        onClick={(e) => {
                          e.stopPropagation();
                          setSelectedAttraction(spot.id);
                        }}
                    >
                        <motion.div layoutId={`image-${spot.id}`} className="absolute inset-0 z-0">
                             <SmoothImage 
                        ...
                        <div className="absolute bottom-0 left-0 w-full p-8 transition-opacity duration-300 group-hover:opacity-0">
                            <div className="flex items-end justify-between">
                                <div>
                                    ...
                                    <motion.h3 layoutId={`title-${spot.id}`} className="text-4xl font-serif text-white font-bold leading-none">{spot.name}</motion.h3>
                                </div>
        ...
        {selectedItem && createPortal(
          <div className="fixed inset-0 z-[12000] flex items-center justify-center p-4">
            ...
            <motion.div 
              ...
              className="relative w-full max-w-6xl bg-zinc-950 rounded-[2rem] overflow-hidden flex flex-col md:flex-row shadow-2xl max-h-[90vh] border border-white/10 z-[13000]"
            >
              
              {/* Image Side */}
              <motion.div layoutId={`image-${selectedItem.id}`} className="w-full md:w-1/2 h-64 md:h-auto relative">
                  <SmoothImage 
                  ...
                  <button 
                      ...
                      className="absolute top-6 left-6 md:hidden z-20 p-3 bg-black/40 text-white rounded-full backdrop-blur-md border border-white/10 hover:bg-white hover:text-black transition-colors"
                  >
                      <X size={20} />
                  </button>
              </motion.div>
              ...
              {/* Content Side */}
              <div className="w-full md:w-1/2 p-8 md:p-16 flex flex-col bg-zinc-950 relative overflow-y-auto">
                  <button 
                      onClick={(e) => {
                        e.stopPropagation();
                        setSelectedAttraction(null);
                      }}
                      className="hidden md:flex absolute top-8 right-8 z-20 w-12 h-12 items-center justify-center bg-zinc-900 text-white hover:bg-white hover:text-black rounded-full transition-all duration-300 border border-white/10"
                  >
                      <X size={24} />
                  </button>
                  ...
                  >
                      ...
                      <motion.h3 layoutId={`title-${selectedItem.id}`} className="text-3xl md:text-6xl font-serif text-white font-bold mb-6 md:mb-8 leading-[0.9]">
                          {selectedItem.name}
                      </motion.h3>
                      
                      <div className="w-16 md:w-20 h-1 bg-nyc-taxi mb-8 md:mb-10"></div>
                      
                      <p className="text-lg md:text-2xl text-gray-200 font-display italic leading-relaxed mb-6 md:mb-8">
