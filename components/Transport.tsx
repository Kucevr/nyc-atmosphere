                ...
                {t.transport.items.map((item, idx) => {
                    ...
                    return (
                        <Reveal key={idx} delay={idx * 150} width="100%">
                            <div className="group relative h-[450px] md:h-[600px] overflow-hidden">
                                {/* Background Image with Zoom */}
                                <div className="absolute inset-0 z-0">
                                    <SmoothImage 
                                        src={item.image} 
                                        alt={item.title} 
                                        className="w-full h-full object-cover transition-transform duration-300 ease-out group-hover:scale-102 filter grayscale brightness-[0.5] group-hover:grayscale-0 group-hover:brightness-85"
                                ...
                                <div className="relative z-10 h-full p-8 md:p-12 flex flex-col justify-between">
                                    ...
                                    <div className="flex justify-between items-start">
                                        <div className="p-2.5 md:p-3 bg-white/10 rounded-lg border border-white/10 text-white group-hover:bg-nyc-taxi group-hover:text-black transition-colors duration-150">
                                    ...
                                    {/* Main Info */}
                                    <div className="transform translate-y-1 group-hover:translate-y-0 transition-transform duration-200 ease-out">
                                        <h3 className="text-3xl md:text-4xl font-serif font-bold text-white mb-2">{item.title}</h3>
                                        <p className="text-gray-300 font-light text-base md:text-lg mb-6 md:mb-8 opacity-85 group-hover:opacity-100 transition-opacity duration-200">
                                            {item.desc}
                                        </p>

                                        {/* Hidden Tip Reveal */}
                                        <div className="overflow-hidden max-h-0 group-hover:max-h-40 transition-all duration-300 ease-out">
                                            <div className="bg-white/10 rounded-xl p-4 border border-white/10 mb-4 opacity-0 group-hover:opacity-100 transition-opacity duration-250 delay-50">
                                        ...
                                        <button className="flex items-center gap-2 text-[10px] md:text-xs font-bold uppercase tracking-widest text-nyc-taxi opacity-0 group-hover:opacity-100 transition-opacity duration-700 delay-200">
